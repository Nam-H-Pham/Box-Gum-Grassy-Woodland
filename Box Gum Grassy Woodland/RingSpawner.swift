...
class RingSpawner {
    ...
    func spawn() -> Entity {
        ...
        let entity = createEntityClone(from: model, lodRange: lodRange, position: position)
    ...
    func spawn(at position: SIMD3<Float>) -> Entity {
        let (modelFilename, lodRange) = selectRandomModelFilenameAndRange()
        let model = loadModel(named: modelFilename)
        let entity = createEntityClone(from: model, lodRange: lodRange, position: position + translation)
        existingPositions.append(position)
        return entity
    }
